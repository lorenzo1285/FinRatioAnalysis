...
2. **Version Bump**: Update version in `pyproject.toml` before each release (currently 0.2.1)
3. **Development Dependencies**: Install with `uv sync --group dev` or `pip install -e ".[mcp]"`

## Method 1: Manual Publishing with uv (Recommended)
...
This creates:
- `dist/FinRatioAnalysis-0.2.1-py3-none-any.whl` (wheel)
- `dist/FinRatioAnalysis-0.2.1.tar.gz` (source)

**Verify the build includes MCP package:**
```bash
# Check wheel contents (PowerShell)
Expand-Archive -Path dist/FinRatioAnalysis-0.2.1-py3-none-any.whl -DestinationPath temp_check -Force
Get-ChildItem temp_check -Recurse | Select-String "finratioanalysis_mcp"
Remove-Item temp_check -Recurse -Force

# Or on Linux/Mac
unzip -l dist/FinRatioAnalysis-0.2.1-py3-none-any.whl | grep finratioanalysis_mcp
```
